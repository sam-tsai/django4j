...
import org.django4j.servlet.DjangoResponse;
...
public class DjangoEngine implements IDjangoApp {
	...
	public void doAction(final IRequest request,
		...
		final InvokerHandle handle = route.getRouter(request);
		try {
			handle.invoke(request, new DjangoResponse(servletResponse, "utf-8"));
